...
Renderer::Renderer(const EngineConfigs &crConfigs, ThreadPool &threadPool)
    : mpConfigs{&crConfigs}, mpThreadPool{&threadPool}, mInstance{*mpConfigs},
      ...
      mCommandBuffers{
          mCommandPool.alloc(vku32(maxFramesInFlight)),
      },
      mSwapchain{
          crConfigs,
          mSurface,
      ...
      mVertexBuffer{
          ...
          mCommandPool,
          vertices.data(),
          sizeof(shader_object::Vertex) * vertices.size(),
      },
      mIndexBuffer{
          mDevice,
          mCommandPool,
          indices.data(),
          sizeof(shader_object::Index) * indices.size(),
      },
      mTextureImage{
          "Data/Resources/Textures/Texture.jpeg",
          mDevice,
          mSwapchain,
          mCommandPool,
      },
      ...
      mDescriptorPool{
          crConfigs,
          mDevice,
          vku32(maxFramesInFlight),
      },
      mDescriptorSets{mDescriptorPool.alloc({
          maxFramesInFlight,
          *mDescriptorSetLayout,
      })},
      mRenderPass{
...
void Renderer::updateDesciptorSets() {
  for (size_t i = 0; i < maxFramesInFlight; ++i) {
    ...
    bufferInfo.offset = 0;
    bufferInfo.range = sizeof(shader_object::Uniform);
